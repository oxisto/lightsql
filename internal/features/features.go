...
var Groups = []Group{
	{
		...
		Features: []Feature{
			...
			{Name: "Identity columns", Parse: Yes, Exec: Partial,
				Note: "GENERATED ALWAYS and GENERATED BY DEFAULT AS IDENTITY on an integer " +
					"column, backed by the same per-column sequence as SERIAL and implying " +
					"NOT NULL. ALWAYS refuses a statement that supplies its own value, as " +
					"PostgreSQL does, but OVERRIDING SYSTEM VALUE is not implemented, so " +
					"there is no way to override it. Sequence options such as START WITH are " +
					"refused rather than ignored",
				Setup: []string{"CREATE TABLE i (id INT PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, v INT)"},
				SQL:   "INSERT INTO i (v) VALUES (1)"},
		},
	},
	{
