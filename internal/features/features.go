...
var Groups = []Group{
	{
		...
		Features: []Feature{
			...
			{Name: "Column constraints", Parse: Yes, Exec: Partial,
				Note:  "NOT NULL, PRIMARY KEY and UNIQUE are enforced; DEFAULT, CHECK and REFERENCES parse but are not applied",
				Setup: []string{"CREATE TABLE t (id INT PRIMARY KEY, e TEXT UNIQUE, n INT NOT NULL)"},
				SQL:   "INSERT INTO t (id, e, n) VALUES (1, 'a', 0)"},
			{Name: "Table constraints", Parse: Yes, Exec: Partial,
				Note:  "PRIMARY KEY and UNIQUE, including over several columns, where the combination must be unique; FOREIGN KEY and CHECK parse only",
				Setup: []string{"CREATE TABLE t (a INT, b INT, c INT, PRIMARY KEY (a, b), UNIQUE (c))"},
