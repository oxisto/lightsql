...
const (
	...
	// ConstraintIdentity is GENERATED { ALWAYS | BY DEFAULT } AS IDENTITY, the
	// standard spelling of what PostgreSQL also offers as SERIAL.
	ConstraintIdentity
)
...
var constraintKindNames = [...]string{
	...
	ConstraintIdentity:   "GENERATED AS IDENTITY",
}
...
type ColumnConstraint struct {
	...
	Ref *ForeignKeyRef
	// IdentityAlways distinguishes GENERATED ALWAYS from GENERATED BY DEFAULT.
	// The two differ only in whether a statement may supply its own value, so
	// they are one constraint kind with a flag rather than two kinds.
	IdentityAlways bool
}
