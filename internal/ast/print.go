...
func (p *printer) node(n Node) {
	...
	case *ColumnConstraint:
		p.open("constraint")
		if n.Kind == ConstraintIdentity {
			// The two identity forms differ only in a flag, so printing the
			// kind alone would render them identically and a golden test could
			// not tell one from the other.
			if n.IdentityAlways {
				p.atom("generated always as identity")
			} else {
				p.atom("generated by default as identity")
			}
		} else {
