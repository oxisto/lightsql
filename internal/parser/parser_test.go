...
func TestParseCreateTable(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			want: "(union (select (items (col a)) (from (table t))) " +
				"(select (items (col b)) (from (table u))))",
		},
		{
			name: "union all",
			sql:  "SELECT a FROM t UNION ALL SELECT b FROM u",
			want: "(union-all (select (items (col a)) (from (table t))) " +
				"(select (items (col b)) (from (table u))))",
		},
		{
			// DISTINCT is the default and may be written out.
			name: "union distinct is plain union",
			sql:  "SELECT a FROM t UNION DISTINCT SELECT b FROM u",
			want: "(union (select (items (col a)) (from (table t))) " +
				"(select (items (col b)) (from (table u))))",
		},
		{
			// INTERSECT binds tighter, so this is a UNION whose right side is
			// the intersection -- not an intersection of the union. Reading the
			// words left to right gives the wrong answer, which is why the
			// whole tree is asserted rather than the top node.
			name: "intersect binds tighter than union",
			sql:  "SELECT a FROM t UNION SELECT b FROM u INTERSECT SELECT c FROM v",
			want: "(union (select (items (col a)) (from (table t))) " +
				"(intersect (select (items (col b)) (from (table u))) " +
				"(select (items (col c)) (from (table v)))))",
		},
		{
			// UNION and EXCEPT share a precedence and associate left.
			name: "union and except associate left",
			sql:  "SELECT a FROM t UNION SELECT b FROM u EXCEPT SELECT c FROM v",
			want: "(except (union (select (items (col a)) (from (table t))) " +
				"(select (items (col b)) (from (table u)))) " +
				"(select (items (col c)) (from (table v))))",
		},
		{
			// Parentheses override it, and are also how an arm gets its own
			// trailing clauses.
			name: "parentheses group the arms",
			sql:  "(SELECT a FROM t UNION SELECT b FROM u) INTERSECT SELECT c FROM v",
			want: "(intersect (union (select (items (col a)) (from (table t))) " +
				"(select (items (col b)) (from (table u)))) " +
				"(select (items (col c)) (from (table v))))",
		},
		{
			// The trailing ORDER BY belongs to the whole operation, not to the
			// arm it follows. Attaching it to the right arm would order that
			// arm and then discard the order when the rows were combined.
			name: "order by belongs to the set operation",
			sql:  "SELECT a FROM t UNION SELECT b FROM u ORDER BY 1 LIMIT 2",
			want: "(union (select (items (col a)) (from (table t))) " +
				"(select (items (col b)) (from (table u))) " +
				"(order-by (term asc (lit 1))) (limit (lit 2)))",
		},
		{
			// A parenthesised arm keeps its own, which is the only way to give
			// one arm an ORDER BY.
			name: "a parenthesised arm keeps its own order by",
			sql:  "(SELECT a FROM t ORDER BY a) UNION SELECT b FROM u",
			want: "(union (select (items (col a)) (from (table t)) (order-by (term asc (col a)))) " +
				"(select (items (col b)) (from (table u))))",
		},
		{
