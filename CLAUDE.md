...
`package: lower-case imperative summary` — Go project convention, never `feat:` / `fix:`.
...
The `commit-msg` hook in `.githooks/` rejects anything else, so this is checked rather
than remembered. Enable it once per clone:

```sh
git config core.hooksPath .githooks
```

Full rules — package naming, `all:`, bodies, PR descriptions — are in
